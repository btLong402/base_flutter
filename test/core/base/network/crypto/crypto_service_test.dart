...
void main() {
  ...
  group('AesCryptoService', () {
    test('should encrypt and decrypt correctly', () async {
      ...
      when(() => mockSecureStorage.read(key: AppConstants.cryptoKeyCreatedAt))
          .thenAnswer((_) async => null);
    ...
    test('should reuse existing keys from storage', () async {
      // Arrange
      const plainText = 'Hello World';
      const fakeKey = 'MTIzNDU2Nzg5MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTI='; // 32 bytes base64
      const fakeIv = 'MTIzNDU2Nzg5MDEyMzQ1Ng=='; // 16 bytes base64
      
      when(() => mockSecureStorage.read(key: 'crypto_aes_key'))
          .thenAnswer((_) async => fakeKey);
      when(() => mockSecureStorage.read(key: 'crypto_aes_iv'))
    ...
    test('should rotate key and update timestamp', () async {
      ...
      // Assert
      verify(() => mockSecureStorage.write(key: AppConstants.cryptoAesKey, value: any(named: 'value'))).called(1);
      verify(() => mockSecureStorage.write(key: AppConstants.cryptoAesIv, value: any(named: 'value'))).called(1);
      verify(() => mockSecureStorage.write(key: AppConstants.cryptoKeyCreatedAt, value: any(named: 'value'))).called(1);
    });
  });
}
